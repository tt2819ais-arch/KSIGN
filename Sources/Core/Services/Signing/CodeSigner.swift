...
enum CodeSigner {
    ...
    static func resign(bin: inout MachOBinary,
        ...
        let pageBits = 12, pageSize = 4096

        let old = bin.codeSignatureCmd()
        // 1. Убираем старый блок подписи (сам load command остаётся — обновим его поля).
        ...
        // 3. Размер CMS не зависит от содержимого CD (digest фиксированной длины),
        //    поэтому измеряем его один раз «пустышкой».
        ...
        var special: [Data?] = Array(repeating: nil, count: specialCount) // индексы 0..4 → слоты 5..1
        // слот 1: Info.plist, слот 2: Requirements, слот 3: CodeResources, слот 4: Entitlements
        special[specialCount - 1] = SHA.sha256(infoPlistData)                    // slot 1
        special[specialCount - 2] = SHA.sha256(reqBlob)                          // slot 2
        if let cr = codeResourcesData { special[specialCount - 3] = SHA.sha256(cr) } // slot 3
        special[specialCount - 4] = SHA.sha256(entBlob)                          // slot 4
        ...
        let cdDigest = SHA.sha256(cd)
        let realCMS = try cms(cd)
        guard realCMS.count <= dummyCMS.count + 64 else {
            throw AppError.invalidFormat("размер CMS изменился некорректно")
        }

        // 9. SuperBlob
        let sb = buildSuperBlob(entries: [
            (0, cd), (2, reqBlob), (5, entBlob), (0x10000, realCMS)
        ])
        precondition(sb.count == sbLen, "расхождение расчёта SuperBlob")
        bin.data.append(sb)
    ...
    private static func buildCodeDirectory(ident: [UInt8], team: [UInt8],
        ...
        put32(UInt32(cdTotalLength)) // length — заполняется ниже фактическим размером
        ...
        // Спец-слоты идут в обратном порядке: слот N первым.
        for i in stride(from: specialCount - 1, through: 0, by: -1) {
            b.append(contentsOf: Array(special[i] ?? Data(repeating: 0, count: 32)))
        }
        for h in pages { b.append(contentsOf: Array(h)) }

        var out = Data(b)
        // корректная общая длина
        var lenBE = BE.bytes(UInt32(out.count))
        out.replaceSubrange(4..<8, with: Data(lenBE))
        _ = lenBE; lenBE = []
        return out
    }

    private static var cdTotalLength: Int { 0 } // длина перезаписывается в конце сборки
