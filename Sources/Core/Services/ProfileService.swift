...
enum ProfileService {
    ...
    /// .mobileprovision — это CMS(PKCS#7)-контейнер, внутри которого лежит plist.
    /// Декодировать CMS-конверт не обязательно: встроенный plist надёжно извлекается
    /// по границам XML (подход, используемый множеством open-source инструментов).
    static func parse(raw data: Data) throws -> ParsedProfile {
        ...
        let pattern = appID.contains(".") ? String(appID.split(separator: ".", maxSplits: 1).last ?? "") : "*"

        var type: ProfileType = .unknown
        let taskAllow = ent["get-task-allow"] as? Bool ?? false
        if dict["ProvisionsAllDevices"] != nil { type = .enterprise }
        else if taskAllow { type = .development }
        else if (dict["ProvisionedDevices"] as? [String])?.isEmpty == false { type = .adHoc }
        else { type = .appStore }
    ...
    private static func embeddedPlist(data: Data) -> Data? {
        guard let start = data.range(of: Data("<?xml".utf8)),
              let end = data.range(of: Data("</plist>".utf8)), end.lowerBound > start.upperBound else { return nil }
