...
final class CertificateService {
    ...
    /// Импортирует p12. Пароль используется ОДИН раз и не сохраняется нигде.
    /// Identity попадает в Keychain средствами SecPKCS12Import (по умолчанию persist).
    ...
    // MARK: - Разбор информации о сертификате (все публичные API)

    static func describe(chain: [SecCertificate], hasKey: Bool) -> CertificateDetails {
        let leaf = chain[0]
        func val(_ oid: CFString) -> String? {
        ...
        let cn = val(kSecOIDCommonName) ?? "Без имени"
        let org = val(kSecOIDOrganizationName) ?? ""
        let ou = val(kSecOIDOrganizationalUnitName) ?? ""
        let issuerCN = val(kSecOIDX509V1IssuerName) ?? ""
        ...
        let serial = (try? SecCertificateCopyNormalizedSubjectSequence(leaf)) // заглушка типов ниже
        _ = serial
        ...
        return CertificateDetails(
            label: cn,
            certType: type,
            teamID: ou.count == 10 && ou.allSatisfy(\.isLetter.isUpperOrDigit) ? ou : ou,
            teamName: org,
