...
enum IPAParser {
    ...
    static func locateAppDir(in root: URL) throws -> URL {
        ...
        let items = try fm.contentsOfDirectory(at: payload.exists ? payload : root,
    ...
    /// Попытка достать PNG-иконку (работает для классических CFBundleIconFiles;
    /// для современных Assets.car — вернёт nil, иконку рисуем плейсхолдером).
    ...
    static func importAndAnalyze(ipaURL: URL, extractRoot: URL,
