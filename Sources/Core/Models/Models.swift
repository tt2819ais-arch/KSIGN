...
final class StoredCertificate {
    ...
    var chainFileName: String     // файл Documents/Certs/<id>.chain (DER-цепочка)
    var hasPrivateKey: Bool
    var importedAt: Date

    var daysLeft: Int { Calendar.current.dateComponents([.day],
        from: Date(), to: notAfter).day ?? 0 }
    ...
    init(/* поля выше */) {
        // инициализация полей — см. вызовы в CertificateService
        self.id = UUID(); self.label = label; self.certType = certType
        self.teamID = teamID; self.teamName = teamName
        self.subjectSummary = subjectSummary; self.issuerSummary = issuerSummary
        self.notBefore = notBefore; self.notAfter = notAfter
        self.serialHex = serialHex; self.certSHA1 = certSHA1
        self.chainFileName = chainFileName; self.hasPrivateKey = hasPrivateKey
...
final class StoredProfile {
    ...
    var profileType: String       // Development / AdHoc / Enterprise / AppStore
    var createdAt: Date
    var expirationDate: Date
    var entitlementsJSON: String
    var devicesJSON: String
    var fileName: String          // Documents/Profiles/<id>.mobileprovision
    var importedAt: Date
